...
struct device media_bus = { .init_name = "media",
...
{
	struct media_device *pdev = to_media_device(dev);
	return (strncmp(pdev->devfs_name, drv->name,
					   ...
					   struct media_ops *ops)
{
	...
	pdrv = kzalloc(sizeof(struct media_driver) +
			       strnlen(name, MEDIA_MAX_DEV_NAME_LEN),
		       GFP_KERNEL);
	if (pdrv == NULL) {
