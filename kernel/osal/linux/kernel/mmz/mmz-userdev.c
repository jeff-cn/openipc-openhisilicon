			   ...
			   struct mmb_info *pmi)
{
	...
	mmb = mmz_mmb_alloc(pmi->mmb_name, pmi->size, pmi->align, pmi->gfp,
			    pmi->mmz_name);
	if (mmb == NULL) {
...
static int ioctl_mmb_alloc_v2(struct file *file, unsigned int iocmd,
			      struct mmb_info *pmi)
{
	struct mmz_userdev_info *pmu = file->private_data;
	struct mmb_info *new_mmbinfo = NULL;
	...
	mmb = mmz_mmb_alloc_v2(pmi->mmb_name, pmi->size, pmi->align, pmi->gfp,
			       pmi->mmz_name, pmi->order);
...
static int ioctl_mmb_attr(struct file *file, unsigned int iocmd,
			  struct mmb_info *pmi)
{
	struct mmz_userdev_info *pmu = file->private_data;
	struct mmb_info *p = NULL;
...
static int ioctl_mmb_virt2phys(struct file *file, unsigned int iocmd,
			       struct mmb_info *pmi)
{
	int ret = 0;
...
static int mmz_userdev_ioctl_r(struct file *file, unsigned int cmd,
			       struct mmb_info *pmi)
{
	switch (_IOC_NR(cmd)) {
...
/* just for test */
static int mmz_userdev_ioctl_t(struct file *file, unsigned int cmd,
			       struct mmb_info *pmi);

static long mmz_userdev_ioctl(struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	int ret = 0;
	struct mmz_userdev_info *pmu = file->private_data;
	...
	if (_IOC_TYPE(cmd) == 'm') {
		...
		if (copy_from_user(&mi, (void *)(uintptr_t)arg,
				   _IOC_SIZE(cmd))) {
		...
		if (!ret && (cmd & IOC_OUT)) {
			if (copy_to_user((void *)(uintptr_t)arg, &mi,
					 _IOC_SIZE(cmd))) {
		...
		if (copy_from_user(&area, (void *)(uintptr_t)arg,
				   _IOC_SIZE(cmd))) {
			printk(KERN_WARNING
			       "\nmmz_userdev_ioctl: copy_from_user error.\n");
			ret = -EFAULT;
		...
		if ((uintptr_t)area.dirty_virt_start !=
		    (uintptr_t)pmi->mapped + offset) {
			printk(KERN_WARNING
			       "dirty_virt_start addr was not consistent with dirty_phys_start addr!\n");
			ret = -EFAULT;
			goto __error_exit;
		}
		if (area.dirty_phys_start + area.dirty_size >
		...
		area.dirty_size =
			(area.dirty_size + (orig_addr - area.dirty_phys_start) +
			 (CACHE_LINE_SIZE - 1)) &
			~(CACHE_LINE_SIZE - 1);
...
#ifdef CONFIG_COMPAT
static long compat_mmz_userdev_ioctl(struct file *file, unsigned int cmd,
				     unsigned long arg)
{
...
static struct miscdevice mmz_userdev = { .minor = MISC_DYNAMIC_MINOR,
