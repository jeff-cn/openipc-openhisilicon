...
#define IO_PIRISI_ADDRESS(dev, x) \
	((GK_UINTPTR_T)reg_pirisI_base_va[dev] + ((x) - (PIRISI_ADRESS_BASE)))
...
#define PIRIS_WRITE_REG(Addr, Value) \
...
static const unsigned char motor_phase_tbl[MAX_MOTOR_PAHSE] = { 0x0, 0x1, 0x2,
								0x3 };
...
#endif
	int
	piris_origin_set(int dev, PIRIS_DATA_S *pstPirisData)
{
	int piris_pos;
	unsigned long u32Flags;
	PIRIS_DEV *pstPiris;
	...
	if ((piris_pos > 200) || (piris_pos < -200) ||
	    (piris_pos == p_piris_dev[dev]->src_pos)) {
...
#endif
	int
	piris_close_set(int dev, PIRIS_DATA_S *pstPirisData)
{
	int piris_pos;
	unsigned long u32Flags;
	PIRIS_DEV *pstPiris;
...
#endif
{
	...
	case PIRIS_SET_FD:
#ifdef __LITEOS__
		if (read_user_linear_space_valid((GK_U8 *)arg, sizeof(int)) ==
		    GK_FALSE) {
			up(&pstPiris->sem);
			return -EFAULT;
		}
...
#else
		if (read_user_linear_space_valid((GK_U8 *)(GK_UINTPTR_T)arg,
						 sizeof(long int)) ==
		    GK_FALSE) {
			up(&pstPiris->sem);
			return -EFAULT;
		}
		ret = copy_from_user(&dev, (long int __user *)(GK_UINTPTR_T)arg,
				     sizeof(long int));
		if (ret) {
			up(&pstPiris->sem);
			return -EFAULT;
	...
	case PIRIS_SET_ACT_ARGS:
		...
		ret = copy_from_user(&PirisPos, (int __user *)(GK_UINTPTR_T)arg,
				     sizeof(int));
		if (ret) {
			up(&pstPiris->sem);
			return -EFAULT;
	...
	case PIRIS_SET_ORGIN:
		...
		ret = copy_from_user(&piris_data,
				     (PIRIS_DATA_S __user *)(GK_UINTPTR_T)arg,
				     sizeof(PIRIS_DATA_S));
		if (ret) {
			up(&pstPiris->sem);
			return -EFAULT;
	...
	case PIRIS_GET_STATUS:
		...
		if (write_user_linear_space_valid((GK_U8 *)(GK_UINTPTR_T)arg,
						  sizeof(PIRIS_STATUS_E)) ==
		    GK_FALSE) {
			up(&pstPiris->sem);
			return -EFAULT;
		}

		ret = copy_to_user((PIRIS_STATUS_E __user *)(GK_UINTPTR_T)arg,
				   &PirisStatus, sizeof(PIRIS_STATUS_E));
		if (ret) {
			up(&pstPiris->sem);
			return -EFAULT;
		}
...
#endif
{
	...
	pstPiris->phase =
		(pstPiris->phase + MAX_MOTOR_PAHSE + sign) % MAX_MOTOR_PAHSE;
...
static int piris_isp_register(int dev)
{
	...
	if ((FUNC_ENTRY(ISP_EXPORT_FUNC_S, MOD_ID_ISP) != NULL) &&
	    CKFN_ISP_RegisterPirisCallBack()) {
		CALL_ISP_RegisterPirisCallBack(dev, &stPirisCb);
	} else {
		osal_printk(
			"register piris_gpio_write_callback to isp failed, piris init is failed!\n");
		return -1;
	}
...
static int piris_isp_unregister(int dev)
{
	...
	if ((FUNC_ENTRY(ISP_EXPORT_FUNC_S, MOD_ID_ISP) != NULL) &&
		...
		osal_printk(
			"unregister piris_gpio_write_callback to isp failed!\n");
		return -1;
	}
	...
	for (i = 0; i < PIRIS_MAX_DEV_NUM; i++) {
		if (!reg_pirisI_base_va[i]) {
			reg_pirisI_base_va[i] =
				osal_ioremap(PIRISI_ADRESS_BASE, 0x10000);
			if (!reg_pirisI_base_va[i]) {
				for (j = 0; j < i; j++) {
...
static struct platform_driver piris_driver = { .probe = piris_probe,
					       .remove = piris_remove,
					       .driver = {
						       .name = "piris",
						       .of_match_table =
