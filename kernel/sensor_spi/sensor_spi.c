...
#endif
#ifdef __LITEOS__
extern int spi_dev_set(int host_no, int cs_no,
		       struct spi_ioc_transfer *transfer); // spi_devdev_set
...
GK_S32 ssp_write(unsigned int spi_bus, unsigned int spi_csn, unsigned int addr1,
		 unsigned int addr1bytenum, unsigned int addr2,
		 unsigned int addr2bytenum, unsigned int data,
		 unsigned int databytenum)
{
	...
	return ssp_write_alt(spi_bus, spi_csn, addr1, addr1bytenum, addr2,
			     addr2bytenum, data, databytenum);
}
...
int sensor_spi_dev_init(void *pArgs)
{
	...
	if (pstSpiModuleParam != NULL) {
		...
		memcpy(sensor, pstSpiModuleParam->cSensor,
		       sizeof(pstSpiModuleParam->cSensor));
	}
	stBusCb.pfnISPWriteSSPData = ssp_write;
	if (CKFN_ISP_RegisterBusCallBack()) {
		for (i = 0; i < ISP_MAX_DEV; i++) {
			CALL_ISP_RegisterBusCallBack(i, ISP_BUS_TYPE_SSP,
						     &stBusCb);
		}
	} else {
	...
	for (bus_num = 0; bus_num < SPI_MAX_NUM; bus_num++) {
		...
		for (csn = 0; csn < spi_csn[bus_num]; csn++) {
			if (master) {
				...
				snprintf(spi_name, sizeof(spi_name), "%s.%d",
					 dev_name(&master->dev), csn);
				dev = bus_find_device_by_name(&spi_bus_type,
							      NULL, spi_name);
				if (dev == NULL) {
					dev_err(NULL,
						"chipselect %d has not been used\n",
						csn);
					status = -ENXIO;
					goto end1;
				}
